...
it takes the raw focal plane  data in binay format.
...
def main():
    ...
    for data_path in data_path_all:
        ...
        quad_A = full_frame[:, 0, :, :]
        active_quad_A = np.mean(quad_A[:, 2:1028, 10:1034], axis=0)        
     
        quad_B = full_frame[:, 1, :, :]
        active_quad_B = np.mean(quad_B[:, 2:1028, 10:1034], axis=0)
      
        quad_C = full_frame[:, 2, :, :]
        active_quad_C = np.mean(quad_C[:, 2:1028, 10:1034], axis=0)
        
        quad_D = full_frame[:, 3, :, :]
        active_quad_D = np.mean(quad_D[:, 2:1028, 10:1034], axis=0)
