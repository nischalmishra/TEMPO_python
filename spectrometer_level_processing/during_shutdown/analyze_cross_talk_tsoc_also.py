...
def main():
    ...
    #nlat = nx_quad*2
    #nspec = ny_quad*2
    file_path = r'D:\TEMPO\Data\GroundTest\FPS\Crosstalk'
    save_file_path = r'C:\Users\nmishra\Workspace\TEMPO\Cross_Talk_Test_After_TVAC\all_files' 
    temp_files = os.listdir(file_path) 
    
    for files in range(1, 4):  
        col_names =  ['quadA', 'quadB', 'quadC','quadD']
...
#                    print(string3)
                ...
                if spot == 1:
                    ...
                    if i == 0: 
                        ...
                        even_tsoc_val = even_tsoc[0, 185:325]
                        odd_tsoc_val = odd_tsoc[0, 185:325]
                        
                        tsoc_A_even.append(even_tsoc_val)                        
                        tsoc_A_odd.append(odd_tsoc_val)
                        #even_tsoc = []
                        #odd_tsoc = []                              
                        ...
                        even_tsoc_val = even_tsoc[1,185:325]
                        odd_tsoc_val = odd_tsoc[1, 185:325]
                        tsoc_B_even.append(even_tsoc_val)
                        tsoc_B_odd.append(odd_tsoc_val)
                        
                    elif i == 2:                        
                        rows_max_C.append(cross_talk_array[185:325, 170:310])
                        even_tsoc_val = even_tsoc[2, 185:325]
                        odd_tsoc_val = odd_tsoc[2, 185:325]
                        tsoc_C_even.append(even_tsoc_val)
                        tsoc_C_odd.append(odd_tsoc_val)
                       
                    elif i == 3:                        
                        rows_max_D.append(cross_talk_array[185:325, 170:310])
                        even_tsoc_val = even_tsoc[3, 185:325]
                        odd_tsoc_val = odd_tsoc[3, 185:325]
                        tsoc_D_even.append(even_tsoc_val)
                        tsoc_D_odd.append(odd_tsoc_val)
                    ...
                    if i==0:
                        ...
                        even_tsoc_val = even_tsoc[0, 720:860]
                        odd_tsoc_val = odd_tsoc[0, 720:860]
                        tsoc_A_even.append(even_tsoc_val)                        
                        tsoc_A_odd.append(odd_tsoc_val)
                       
                    elif i==1:
                        rows_max_B.append(cross_talk_array[720:860, 720:860])
                        even_tsoc_val = even_tsoc[1, 720:860]
                        odd_tsoc_val = odd_tsoc[1, 720:860]
                        tsoc_B_even.append(even_tsoc_val)
                        tsoc_B_odd.append(odd_tsoc_val)
                       
                    elif i==2:
                        rows_max_C.append(cross_talk_array[720:860, 720:860])
                        even_tsoc_val = even_tsoc[2, 720:860]
                        odd_tsoc_val = odd_tsoc[2, 720:860]
                        tsoc_C_even.append(even_tsoc_val)
                        tsoc_C_odd.append(odd_tsoc_val)
                        
                    elif i==3:
                        rows_max_D.append(cross_talk_array[720:860, 720:860])
                        even_tsoc_val = even_tsoc[3, 720:860]
                        odd_tsoc_val = odd_tsoc[3, 720:860]
                        tsoc_D_even.append(even_tsoc_val)
                        tsoc_D_odd.append(odd_tsoc_val)
                 
            #print(np.array(tsoc_A_even)-np.array(tsoc_B_even))
            #print(tsoc_B_even)
            #print(np.array(tsoc_A_even).shape, np.array(tsoc_B_even).shape)
            #plt.plot(tsoc_C_even, tsoc_D_even,'bo')
            #plt.show()
            
            
                
               # tsoc_all = [tsoc_A_even), tsoc_B_even, tsoc_C_even, tsoc_D_even]
            dframe1 = pd.DataFrame(np.array(tsoc_A_even).T)
            print(dframe1.head())
            cc
            dframe1 = [dframe1, pd.DataFrame(np.array(tsoc_B_even).T)]
            dframe1 = pd.concat(dframe1, axis=1)
           
            dframe1 =  [dframe1, pd.DataFrame(np.array(tsoc_C_even).T)]
            dframe1 = pd.concat(dframe1, axis=1)
            dframe1 = [dframe1, pd.DataFrame(np.array(tsoc_D_even).T)]
            dframe1 = pd.concat(dframe1, axis=1)           
            dframe1.columns = ['quadA', 'quadB', 'quadC','quadD']    
            ...
            df_name = os.path.join(save_dir_tsoc,data_files+'.csv')            
            dframe1.to_csv(df_name, sep=',', float_format='%1.2f')
