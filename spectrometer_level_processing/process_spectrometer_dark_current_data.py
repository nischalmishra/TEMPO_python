...
def extract_active_region(bias_subtracted_quads):
    ...
    for quads in range(0, num_quads):
        active_quad = bias_subtracted_quads[quads, 100:900, 100:900].astype('float')
       
        #active_quad[active_quad >= 0.90*16383] = np.NAN
        odd_detector_active = active_quad[ :, ::2]
        even_detector_active = active_quad[:, 1::2]
        
        all_quads_even.append(np.mean(filter_outlier_median(even_detector_active)))
        all_quads_odd.append(np.mean(filter_outlier_median(odd_detector_active)))
...
def main():
    ...
    telemetry_dir = os.path.join(r'F:\TEMPO\Data\GroundTest\FPS\Spectrometer\2018.06.27\processed\h5')
    image_dir = os.path.join(data_dir, 'saved_quads\Dark_Current_data')
    image_save_dir = os.path.join(image_dir, 'saved_DC')
...
#            fpa_temp = -21.5
       ...
       # print('Max. Val. Raw = ', np.max(raw_image))
        ...
        unsmeared_quads = perform_smear_removal(np.array(bias_removed_quads), int_time)
        even_quad_active, odd_quad_active = extract_active_region(np.array(unsmeared_quads))
    ...
    fpa_temp = os.path.join(image_save_dir, 'fpa_temp.csv')
    ...
    np.savetxt(fpa_temp, fpa_temp_all, fmt='%1.3f', delimiter=",")
