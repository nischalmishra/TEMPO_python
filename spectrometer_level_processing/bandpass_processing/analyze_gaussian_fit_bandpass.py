...
#def flat_top_gaussian_UV(wavelength,amp, center, width ):
#    """
#    Fit the flat top Gaussian FUnction    """
#    
#    gauss1 = amp * np.exp(-(wavelength-center)**4/ 2*width**4)    
#    sum_gauss = gauss1
#    return sum_gauss
...
def smooth_data_interpol(wavelen, bp_data):
    """ smooth the data with linear interpolation
    """
...
def  fit_double_gaussian_func(bandpass_data, bandpass_wvl, save_dir, wavelen_val):
     ...
     for i in range(1500, np.shape(bandpass_data)[1]):
...
def main():
    ...
    for files in range(0, len(all_wavelengths_file)):
        ...
        bandpass_data = np.genfromtxt(os.path.join(bp_data_path,'bandpass_radiance_normalized_V1.csv'),
                                      delimiter=',')
