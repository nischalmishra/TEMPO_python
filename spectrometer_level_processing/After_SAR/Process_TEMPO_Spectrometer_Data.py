...
def calculate_median_offset(raw_quads):
    ...
    for quads in range(0, num_quads):
        ...
        rows, cols = even_detector_bias.shape
        even_detector_bias =even_detector_bias [:, 0:cols-1]
        avg_bias_even = np.mean(odd_detector_bias, axis=1)
        
        print(avg_bias_odd - avg_bias_even)
...
def perform_bias_subtraction(raw_quads):
    ...
    for quads in range(0, num_quads):
        ...
        #print(np.mean(odd_detector_bias))
...
def apply_linearity_correction(active_quad, quad_name, num_coadds):
    ...
    #print(np.max(np.multiply(linearized_quad, num_coadds)))
    return  linearized_quad

def perform_smear_removal(active_quads, int_time, outlier_masks):
...
def apply_electronics_gain(full_frame):
    ...
    electronics_gain_even = [0.0602, 0.0599, 0.0605, 0.0608]
    all_quads = []
    num_quads, nx_quad, ny_quad = full_frame.shape
    for quads in range(0, num_quads):
        active_quad = full_frame[quads, :, :]   
        #print(np.max(active_quad))
