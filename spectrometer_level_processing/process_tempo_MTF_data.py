...
def create_image(image_data, title, figure_name):
    ...
    #plt.grid(False)
    fig_ax.set_xlim(10, 22)
    fig_ax.set_ylim(490, 502)
    #plt.show()
...
def main():
    ...
    """
    data_dir = r'F:\TEMPO\Data\GroundTest\FPS\Spectrometer\2018.04.20 NS and EW MTF\NS_MTF\390_nm_px'
...
#    #dark_data = data_path_all[1:len(data_path_all):2]
    ...
    #data_path_all = [data_path_all[10], data_path_all[25], data_path_all[45]]
...
#    resultFile.close()
    if dark_current:
        dark_current = calculate_dark_current(image_dir, coadds=10)
    else:
        dark_current = 0
...
#        cc
       ...
       #################### Telemetry Statistics###############################
        ...
        telemetry_file = os.path.join(telemetry_dir, telemetry_file_name+'.ch5')
       ...
       #########################NON LINEARITY CORRECTION#######################
        ...
        processed_image = processed_image/(10*prnu_spectrometer)
        #processed_image[processed_image >= 0.85*16383] = 0
...
#        mean_image.extend((processed_image[1041, 1019], processed_image[1041, 1020], processed_image[1041, 1021],
#                           processed_image[1041, 1022], processed_image[1041, 1023], processed_image[1041, 1024],
#                           processed_image[1041, 1025], processed_image[1041, 1026], processed_image[1041, 1027]))
        mean_image.extend((processed_image[497, 19], processed_image[497, 18], processed_image[497, 17],
                           processed_image[497, 16], processed_image[497, 15], processed_image[497, 14],
                           processed_image[497, 13], processed_image[497, 12], processed_image[497, 11]))

        plt.plot(mean_image,'ro--')
        plt.title('WL = ' + str(wavelen) +' nm, ' +' pin hole loc = px', fontsize=12)
        plt.xlabel('Pixel Number', fontsize=12)
        plt.ylabel('Pixel Response (DN)', fontsize=12)
        plt.ylim(-500, 8500)
        plt.grid(linestyle=':')
        plot_save_dir = os.path.join(plot_dir, 'pixel_response')
        plt.savefig(plot_save_dir+'/'+data_path+'.png', dpi=100, bbox_inches="tight")       
        plt.close('all')
        mean_image= []
        
        mean_image1.append(processed_image)               
       
        count = count+1
    text1 =  'Mean Image ('+ str(int(count)) +  'Images)' +' (Laser WL:' + \
                           str(wavelen) +' nm)\n Int. time:' + str(coadds)+ \
                           'ms, Co-adds:' +str(int(coadds))+\
                           ', FPE temp:'+ str(round(fpe_temp, 1))+'C, ' + \
                           ' FPA temp: ' + str(round(fpa_temp, ))+'C'
    mean_image1 = np.sum(mean_image1, axis=0)
    plot_save_dir = plot_dir + '/' +'Mean_MTF_image.png'
    #create_image(mean_image1, text1, plot_save_dir)
    MTF_image_name = plot_dir +'/'+ 'mean_MTF_data_rev1.csv'
   # np.savetxt(MTF_image_name, np.array(mean_image), delimiter=",")
    print('DONE!!')
