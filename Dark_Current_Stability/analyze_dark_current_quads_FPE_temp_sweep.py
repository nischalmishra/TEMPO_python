...
def create_hist(dark_current, title, figure_name, COLOR):
    """ Create histogram of dark current
    """
...
#    storage = np.reshape(storage, (rows*cols, 1))
    ...
    label1 = 'Stdev = '+ str(round(np.std(dark_current), 2))+ 'DN'
   
    fig, ax = plt.subplots(nrows=1, ncols=1, figsize=(8, 5))
    fig.subplots_adjust(left= 0.125, right=0.95, bottom=0.1, top=0.9,
                        wspace=0.3, hspace=.25)
    ax.hist(dark_current, bins=57,facecolor=COLOR, label=label1)
...
def main():
    ...
    save_dir_local_image = r'C:\Users\nmishra\Workspace\TEMPO\Storage_region_analysis\Image_Sto_Comparisons_FPE_Temp\Image'
    save_dir_local_sto = r'C:\Users\nmishra\Workspace\TEMPO\Storage_region_analysis\Image_Sto_Comparisons_FPE_Temp\Storage'
            ...
            for data_files in nominal_int_files[1:]:
                    ...
                    for i in range(0, 4): # 4 quads
                        ...
                        if num_files == 0:
                            ...
                            print('Analyzing SR Region')
                            active_quad = np.mean(quad[:, 0:2, 10:1034], axis=0)                            
                            tsoc = np.mean(quad[:, 0:2 , 1034:1056], axis=0)
                            tsoc= np.array(tsoc)                            
                            ...
                            plt.plot(active_quad[0, :].T, 'b', label='first line')
                            plt.plot(active_quad[1, :].T, 'r', label='second line')
                            plt.legend(loc='best')
                            plt.title('Profile of first two lines of IR, Quad A @ 15C, Int. Time = 118 ms')
                            plt.ylabel('Raw Counts')
                            plt.xlabel('Pixels Indices (#)')
                            plt.show()
...
#            plot_few_tsocs(all_tsoc_image, all_tsoc_storage, )
            ...
            dframe1 = pd.DataFrame(
                            {'Int_time.' : all_int_time,
                             ...
                             'Var_Quad_B': all_std_quad_A,
                             'Var_Quad_C': all_std_quad_A,
                             'Var_Quad_D': all_std_quad_A,
