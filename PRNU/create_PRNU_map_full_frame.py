...
def main():
    ...
    for data_files in nominal_int_files:
        ...
        orig_image_dir = 'saved_images'
        plot_dir = os.path.join(save_dir, orig_image_dir)
        if not os.path.exists(plot_dir):
           os.makedirs(plot_dir) 
           
       #Let's plot the image of the active quad 
        figure_name= plot_dir+'/'+ string1+ str(int_time)+'_.png'
        title = 'Full Active Area Image, ' + string2 + str(int_time)
        active_image = create_final_image(all_quads)
        create_image(active_image, title, figure_name)

        #******************************************************************
        # The now divide each pixel by scene mean. This will be one of the
        #ways to generate a PRNU mask
        mean_val = np.mean(filter_outlier_median(active_image))
        
        if mean_val == 0:
            mean_val = np.median(active_image)
        PRNU_map = np.true_divide(active_image, mean_val)
        
        PRNU_plot_name = 'PRNU_plots_spectral_dir'
        PRNU_dir = os.path.join(save_dir, PRNU_plot_name)
        ...
        image_dir = os.path.join(save_dir, PRNU_image_dir)
        if not os.path.exists(image_dir):
            os.makedirs(image_dir)
        hist_dir = os.path.join( save_dir, PRNU_hist_dir)
        if not os.path.exists(hist_dir):
            os.makedirs(hist_dir)
   
        #plot the PRNU map
        figure_name  = image_dir+'/'+ string1+ str(int_time)+'_.png'
        title = 'PRNU Map, ' + string2 + str(int_time)+ r" $\mu$" +'secs'
        create_image(PRNU_map, title, figure_name)
        
        csv_file_name = image_dir+'/'+ string1 + str(int_time)+'_PRNU.csv'
