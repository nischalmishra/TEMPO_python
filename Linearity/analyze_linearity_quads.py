...
def perform_bias_subtraction(active_quad, trailing_overclocks):
    ...
    plt.plot(avg_bias_even, 'k.')
    plt.plot(odd_detector_bias,'b.')
    plt.title('Trailing Serial Overclock Plot, Quad B, Int. time = 125047 us')
...
#    cc
    ...
    subset_pixels_begin = [0, 500, 800, 0]
    subset_pixels_end = [100, 700, 900, 1026]
    file_name = ['first_100_WVL','Middle_200_WVL','Last_100_WVL', 'All_quads']
    for j in range(3,4):
         all_int_time = []
         all_med_quad_A_odd = []
         all_med_quad_B_odd = []
         ...
         for data_files in nominal_int_files[5:]:
            ...
            if 'Intensity_Sweep' in file_path:
                ...
                begin = subset_pixels_begin[j]
                end = subset_pixels_end[j]
                for i in range(0, 4): # 4 quads                    

                    quad = all_full_frame[:, 1, :, :]
                    ...
                    bias_subtracted_quad_odd = bias_subtracted_quad_odd[begin:end, :]
                    bias_subtracted_quad_even = bias_subtracted_quad_even[begin:end, :]
         ...
         dframe1.to_csv(plot_dir+'/'+'Linearity_Integration_Sweep'+file_name[j]+ '.csv')
            #dframe1 = pd.DataFrame()
