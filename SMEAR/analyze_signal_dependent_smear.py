...
def main():
    ...
    all_avg_smear_quad_A_odd = []
    all_avg_smear_quad_B_odd = [] 
    all_avg_smear_quad_C_odd = []
    all_avg_smear_quad_D_odd = [] 
    all_avg_smear_quad_A_even = []
    all_avg_smear_quad_B_even = [] 
    all_avg_smear_quad_C_even = []
    all_avg_smear_quad_D_even = [] 
    
    all_std_smear_quad_A_odd = []
    all_std_smear_quad_B_odd = []
    all_std_smear_quad_C_odd = []
    all_std_smear_quad_D_odd = []
    all_std_smear_quad_A_even = []
    all_std_smear_quad_B_even = []
    all_std_smear_quad_C_even = []
    all_std_smear_quad_D_even = []
    ...
    for data_files in nominal_int_files:           
            ...
            smear_A  = np.mean(quad_A[:, 1028:1046, 10:1034], axis=0)
            bias_subtracted_quad = (np.mean(filter_outlier_median(perform_bias_subtraction_ave(active_quad_A, bias_A))))
            avg_smear_A_even = np.median(filter_outlier_median(smear_A[:,::2]))
            avg_smear_A_odd = np.median(filter_outlier_median(smear_A[:,1::2]))       
            all_med_quad_A_odd.append(bias_subtracted_quad)
            all_avg_smear_quad_A_odd.append(avg_smear_A_odd)
            all_std_smear_quad_A_odd.append(np.std(filter_outlier_median(smear_A[:, 1::2])))
            all_med_quad_A_even.append(bias_subtracted_quad)
            all_avg_smear_quad_A_even.append(avg_smear_A_even)
            all_std_smear_quad_A_even.append(np.std(filter_outlier_median(smear_A[:, ::2])))                 
            ...
            smear_B  = np.mean(quad_B[:, 1028:1046, 10:1034], axis=0)
            bias_subtracted_quad = (np.mean(filter_outlier_median(perform_bias_subtraction_ave(active_quad_B, bias_B))))
            avg_smear_B_even = np.median(filter_outlier_median(smear_B[:,::2]))
            avg_smear_B_odd = np.median(filter_outlier_median(smear_B[:,1::2]))         
            all_med_quad_B_odd.append(bias_subtracted_quad)
            all_avg_smear_quad_B_odd.append(avg_smear_B_odd)
            all_std_smear_quad_B_odd.append(np.std(filter_outlier_median(smear_B[:, 1::2])))
            all_med_quad_B_even.append(bias_subtracted_quad)
            all_avg_smear_quad_B_even.append(avg_smear_B_even)
            all_std_smear_quad_B_even.append(np.std(filter_outlier_median(smear_B[:, ::2])))          
            bias_subtracted_quad = None
            # For quad C
            quad_C = all_full_frame[:, 2, :, :]
            active_quad_C = np.mean(quad_C[:, 4:1028, 10:1034], axis=0)
            bias_C = np.mean(quad_C[:, 4:1028, 1034:1056], axis=0) 
            smear_C  = np.mean(quad_C[:, 1028:1046, 10:1034], axis=0)
            bias_subtracted_quad = (np.mean(filter_outlier_median(perform_bias_subtraction_ave(active_quad_C, bias_C))))
            avg_smear_C_even = np.median(filter_outlier_median(smear_C[:,::2]))
            avg_smear_C_odd = np.median(filter_outlier_median(smear_C[:,1::2]))            
            all_med_quad_C_odd.append(bias_subtracted_quad)
            all_avg_smear_quad_C_odd.append(avg_smear_C_odd)
            all_std_smear_quad_C_odd.append(np.std(filter_outlier_median(smear_C[:, 1::2])))
            all_med_quad_C_even.append(bias_subtracted_quad)
            all_avg_smear_quad_C_even.append(avg_smear_C_even)
            all_std_smear_quad_C_even.append(np.std(filter_outlier_median(smear_C[:, ::2])))                    
            bias_subtracted_quad = None
            
            quad_D = all_full_frame[:, 3, :, :]
            active_quad_D = np.mean(quad_D[:, 4:1028, 10:1034], axis=0)
            bias_D = np.mean(quad_D[:, 4:1028, 1034:1056], axis=0) 
            smear_D  = np.mean(quad_D[:, 1028:1046, 10:1034], axis=0) 
            bias_subtracted_quad = (np.mean(filter_outlier_median(perform_bias_subtraction_ave(active_quad_D, bias_D))))
            avg_smear_D_even = np.median(filter_outlier_median(smear_D[:,::2]))
            avg_smear_D_odd = np.median(filter_outlier_median(smear_D[:,1::2]))            
            all_med_quad_D_odd.append(bias_subtracted_quad)
            all_avg_smear_quad_D_odd.append(avg_smear_D_odd)
            all_std_smear_quad_D_odd.append(np.std(filter_outlier_median(smear_D[:, 1::2])))
            all_med_quad_D_even.append(bias_subtracted_quad)
            all_avg_smear_quad_D_even.append(avg_smear_D_even)
            all_std_smear_quad_D_even.append(np.std(filter_outlier_median(smear_D[:, ::2])))  
            bias_subtracted_quad = None
    ...
    dframe1 = pd.DataFrame(
                    {'Int_time.' : all_int_time,
                     ...
                     'Avg_Offset_Quad_A_odd' : all_avg_smear_quad_A_odd,
                     'Avg_Offset_Quad_A_even':all_avg_smear_quad_A_even,
                     'Avg_unct_quad_A_odd' :all_std_smear_quad_A_odd, 
                     'Avg_unct_quad_A_even' :all_std_smear_quad_A_even,
                     
                     'Avg_Quad_B_odd' : all_med_quad_B_odd,
                     'Avg_Quad_B_even' : all_med_quad_B_even,
                     'Avg_Offset_Quad_B_odd' : all_avg_smear_quad_B_odd,
                     'Avg_Offset_Quad_B_even':all_avg_smear_quad_B_even,
                     'Avg_unct_quad_B_odd' :all_std_smear_quad_B_odd, 
                     'Avg_unct_quad_B_even' :all_std_smear_quad_B_even,
                                          
                     'Avg_Quad_C_odd' : all_med_quad_C_odd,
                     'Avg_Quad_C_even' : all_med_quad_C_even,
                     'Avg_Offset_Quad_C_odd' : all_avg_smear_quad_C_odd,
                     'Avg_Offset_Quad_C_even':all_avg_smear_quad_C_even,
                     'Avg_unct_quad_C_odd' :all_std_smear_quad_C_odd, 
                     'Avg_unct_quad_C_even' :all_std_smear_quad_C_even,
                     
                     'Avg_Quad_D_odd' : all_med_quad_D_odd,
                     'Avg_Quad_D_even' : all_med_quad_D_even,
                     'Avg_Offset_Quad_D_odd' : all_avg_smear_quad_D_odd,
                     'Avg_Offset_Quad_D_even':all_avg_smear_quad_D_even,
                     'Avg_unct_quad_D_odd' :all_std_smear_quad_D_odd, 
                     'Avg_unct_quad_D_even' :all_std_smear_quad_D_even,
                     })
                
    dframe1.to_csv(file_path+'/'+'Signal_dependent_smear.csv')
